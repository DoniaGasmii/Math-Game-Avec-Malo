...
def times_table_question(level):
    ...
    ans = table * b
    return {"q": f"{table} × {b} = ?", "a": ans, "exp": f"Multiply: {table} × {b} = {ans}."}
...
with hud3:
    if st.session_state.lives is not None:
        ...
        if st.session_state.time_left <= 0 and st.session_state.started:
            ...
            end_game()
            st.session_state._advance_now = True  # trigger refresh to show end state
...
- Answers accept integers, decimals, simple fractions like `3/4`, and percentages like `25%` where appropriate.
    ...
    with bcol2:
        ...
        if st.button("⏭️ Skip" + ("" if can_skip else " (Practice only)"), disabled=not can_skip):
            ...
            st.session_state._advance_now = True  # set rerun flag
    with bcol3:
        st.write("")
            ...
            if is_correct:
                ...
                if st.session_state.lives is not None:
                    ...
                    if st.session_state.lives <= 0:
                        ...
                        end_game()
                        st.session_state._advance_now = True  # show game over immediately

            # Advance if still running
            if st.session_state.started and (st.session_state.lives is None or st.session_state.lives > 0):
                next_question(level, topic)
                st.session_state._advance_now = True  # schedule rerun outside the form
    ...
    if st.session_state.history:
        ...
        for row in st.session_state.history[-8:][::-1]:
            correct_fmt = row["correct"] if isinstance(row["correct"], str) else (
                format_fraction(row["correct"]) if isinstance(row["correct"], Fraction) else row["correct"]
            )
            st.markdown(f"- {row['result']} **{row['q']}** — Your answer: `{row['your']}` • Correct: `{correct_fmt}`")

st.divider()
st.caption("Tip: you can accept answers like `3/4` (fractions) and `25%` (percentages) when it makes sense.")

# ---- Rerun trigger placed OUTSIDE the form ----
